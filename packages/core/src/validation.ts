...
function findTarget(
  ...
  const element = observation.elements.find(
    (entry) => entry.id === action.target.elementId
  );
  if (!element) {
    throw new Error(
      `Action target '${action.target.elementId}' does not exist in current observation.`
    );
  }
  return element;
}
...
function assertInteractive(action: BrowserAction, element: ElementObservation) {
  ...
  if (
    ...
    throw new Error(
      `Fill target '${element.id}' must be an editable textbox.`
    );
  }
...
function actionRisk(
  ...
  if (["goto", "click", "fill", "press", "select"].includes(action.type)) {
    ...
    if (/delete|remove|destroy|purchase|pay|publish|send/i.test(name))
      return "destructive";
    return "write";
  }
  return "read";
...
function validateNavigation(url: string, policy: ActionPolicy) {
  ...
  if (!policy.allowedOrigins.includes(origin)) {
    throw new Error(
      `Navigation to origin '${origin}' is not allowed by policy.`
    );
  }
}
...
function validateApproval(
  action: BrowserAction,
  element: ElementObservation | undefined,
  policy: ActionPolicy
) {
...
export function validateAction(
  ...
  if (
    ["click", "fill", "press", "select", "hover", "extract"].includes(
      parsed.type
    ) &&
    element
  ) {
